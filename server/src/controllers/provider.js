...
exports.registerProvider = async (req, res) => {
  try {
    ...
    if (isUser)
      return res
        .status(400)
        .json({ message: "Invalid email! Email exists as user" });
    const data = {
      name,
      email,
      password,
    ...
    // const isAuthorized = await providerModel({ isAuthorized });
    let subject = "New Provider Registration";
    let message = `Name: ${data.name}\n Email: ${data.email} \n Address:${data.address} \n Requested to registered`;
    await sendEmail({ email: "noreply@devcamper.io", subject, message });
