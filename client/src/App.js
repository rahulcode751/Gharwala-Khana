...
const PrivateRoute = ({ children }) => {
  ...
  // console.log(provider.provider.isAuthorized);
  return provider && provider.isProvider && provider.provider.isAuthorized ? (
    children
  ) : (
    <Navigate to="/" />
  );
...
function App() {
  ...
  const provider = useSelector((state) => state.provider);
  useEffect(() => {
    if (provider.isProvider) navigate("/provider/dashboard");
  }, [provider]);
  return (
    <Routes>
